...
data class TelemetryDto(
    ...
    fun toDomain(): Telemetry {
        ...
        val mappedWaterLevel = if (waterLevel < 0 || waterLevel > 100 || waterLevel == 9999) {
            TelemetryValue.Unavailable
        } else {
            TelemetryValue.Available(waterLevel, now)
        }
        
        val mappedFlowRate = if (flowRate.isNaN() || flowRate < 0f) {
            TelemetryValue.Unavailable
        } else {
            TelemetryValue.Available(flowRate, now)
        }
        
        val mappedDistance = if (distanceCm.isNaN() || distanceCm < 0f || distanceCm > 1000f) {
            TelemetryValue.Unavailable
        } else {
            TelemetryValue.Available(distanceCm, now)
        }
        
        return Telemetry(
            waterLevel = mappedWaterLevel,
            flowRate = mappedFlowRate,
            distanceCm = mappedDistance
        )
    }
}
