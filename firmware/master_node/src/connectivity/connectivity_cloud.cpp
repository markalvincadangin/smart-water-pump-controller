...
void readDeviceConfigFromFirebase() {
  ...
  // Optional runtime debug log level (0-4).
...
bool readFirebaseControl() {
  ...
  // Use Firebase self-clear as the idempotency guard. This keeps emergency_stop
  // reliable after soft resets even when a stale edge state exists locally.
  ...
  if (jd.success) {
    bool v = jd.boolValue;
    // Do not rely on edge-detection state for operator-critical reset_stop.
    if (v) {
  ...
  // countdown_stop one-shot: stop active timer without changing mode.
...
bool pushFirebaseStatus() {
  ...
  // Omit level until the first valid frame.
  if (waterLevelPct >= 0) {
  ...
  // Push countdown_active so the dashboard can distinguish an expired/stopped timer
  // from a timer that has never started.
