  ...
  for (int attempt = 0; attempt < RS485_MAX_RETRIES && !gotFrame; attempt++) {
    ...
    if (!rs485ReadFrame(payload, sizeof(payload), frameTimeoutMs)) {
      if (attempt == (RS485_MAX_RETRIES - 1)) {
        app_logger.logCloudEvent(APP_LOG_LEVEL_WARN, LogCategory::NETWORK, EventCode::EVT_RS485_TIMEOUT, "ReadFrame Timeout/Fail (all retries)");
      }
      delay(4);
      continue;
    }
    if (!validateSensorResponseFrame(payload)) {
      if (attempt == (RS485_MAX_RETRIES - 1)) {
        app_logger.logCloudEvent(APP_LOG_LEVEL_WARN, LogCategory::NETWORK, EventCode::EVT_RS485_INVALID, "Frame structure invalid (all retries).");
      }
      delay(4);
      continue;
    }
    if (!parseSensorFrameStrict(payload, lvl, flow, err, seq, ldsc)) {
      if (attempt == (RS485_MAX_RETRIES - 1)) {
        app_logger.logCloudEvent(APP_LOG_LEVEL_ERROR, LogCategory::NETWORK, EventCode::EVT_RS485_INVALID, "Parse strict failed after retries. Payload: %s", payload);
      }
      delay(4);
      continue;
    }
