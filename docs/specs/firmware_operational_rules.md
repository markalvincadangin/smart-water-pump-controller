...
- On normal boot, stale `countdown_start` intents are cleared from the cloud shadow before pump logic evaluates. This serves as a software defense-in-depth against rapid-cycling loops in the event of an unexpected hardware brownout (e.g., if the contactor RC snubber fails).
