...
1. **Preview the exact reset:**
   ```bash
   cd functions
   npm run reset-test-env -- --project smartflow-fed87 --database-url https://smartflow-fed87-default-rtdb.asia-southeast1.firebasedatabase.app
   ```
...
2. **Apply only after reviewing the plan.** For a clean ESP32 provisioning test, reseed the one required non-secret bootstrap registry entry after the root wipe:
   ```bash
   npm run reset-test-env -- --project smartflow-fed87 --database-url https://smartflow-fed87-default-rtdb.asia-southeast1.firebasedatabase.app --apply --acknowledge-delete-all-test-data --seed-device-id SF-67D42C --seed-secret-name projects/smartflow-fed87/secrets/smartflow-bootstrap-sf-67d42c
   ```
...
```bash
npm run migrate-legacy-ownership -- --project smartflow-fed87 --database-url https://smartflow-fed87-default-rtdb.asia-southeast1.firebasedatabase.app
```

Apply a reviewed plan:

```bash
npm run migrate-legacy-ownership -- --project smartflow-fed87 --database-url https://smartflow-fed87-default-rtdb.asia-southeast1.firebasedatabase.app --apply --acknowledge-ownership-migration
```
...
```bash
npm run resolve-ownership-conflict -- --project smartflow-fed87 --database-url https://smartflow-fed87-default-rtdb.asia-southeast1.firebasedatabase.app --device-id SF-67D42C --owner-uid <durable-owner-uid> --operator-uid <admin-uid> --evidence CASE-123
```

Apply a reviewed resolution:

```bash
npm run resolve-ownership-conflict -- --project smartflow-fed87 --database-url https://smartflow-fed87-default-rtdb.asia-southeast1.firebasedatabase.app --device-id SF-67D42C --owner-uid <durable-owner-uid> --operator-uid <admin-uid> --evidence CASE-123 --apply --acknowledge-ownership-resolution
```
